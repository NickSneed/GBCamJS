...
import { applyPalette, getPalette } from './palette.js';
...
describe('palette', () => {
    describe('getPalette', () => {
        it('should return the correct palette object for a valid ID', () => {
            const palette = getPalette('test-palette');
            expect(palette.name).toBe('Test Palette');
            expect(palette.colors).toHaveLength(4);
        });

        it('should return the default sgb2h palette if ID is not found', () => {
            const palette = getPalette('invalid-id');
            expect(palette.name).toBe('Super Game Boy 2H');
        });
    });

    describe('applyPalette', () => {
        it('should throw an error if an ID string is passed instead of a palette object', () => {
            const pixels = [0, 1];
            expect(() => applyPalette(pixels, 'test-palette', 'none')).toThrow(/applyPalette requires a full palette object/);
        });
